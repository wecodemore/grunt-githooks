...
module.exports = {
  ...
  createHook: function (hookName, task, options) {
    ...
    if(this.MARKER_REGEX.test(existingCode)) {
      hookContent = this.insertBindingCode(task, existingCode, options.template);
    } else {
      hookContent = this.appendBindingCode(task, existingCode, options.template);
    }
  ...
  validateScriptLanguage: function (script) {
      ...
      if (!this.isValidScriptLanguage(script)) {
   ...
   */
  appendBindingCode: function (task, existingCode, templatePath) {

    var bindingCode = this.createBindingCode(task, templatePath);
    return (existingCode || '#!/usr/bin/env node') +
            '\n\n' +
            bindingCode;
  },
   ...
   * @param templatePath {String}
   */
  insertBindingCode: function (task, existingCode, templatePath) {

    var bindingCode = this.createBindingCode(task, templatePath, true);
